...
export const getUsuariosDoEvento = async (eventoId: number) => {
  try {
    const response = await api.get(`/eventos/${eventoId}/usuarios`);
    ...
    console.error(`Erro ao buscar usuários do evento ${eventoId}:`, error);

    if (error.response) {
      throw new Error(error.response.data?.message || "Erro ao buscar usuários do evento.");
...
export const sendCertificadoPorEmail = async (to: string) => {
  try {
    const response = await api.post("/email/send-certificado", { to });
    ...
    console.error(`Erro ao enviar certificado para ${to}:`, error);

    if (error.response) {
      throw new Error(error.response.data?.message || "Erro ao enviar certificado.");
    } else if (error.request) {
      throw new Error("Servidor não respondeu. Tente novamente mais tarde.");
    } else {
      throw new Error("Erro inesperado ao enviar certificado.");
    }
  }
};
